	...
	db.query(
		...
		function(err, rows) {
			...
			if (rows.length>0)
			{
			    ...
			    db.query(
				  "INSERT INTO accounts (username, lat, lon, city) " + 
				  "VALUES (?, ?, ?, ?)",
				  [work.username, work.lat, work.lon, work.city], 
				  function(err) {
					if (err) throw err;
					res.writeHead(200, {"Content-Type": "application/json"});
					var json = JSON.stringify ({
...
exports.confirm_info = function(db, req, res, time_out){
	exports.parseReceivedData(req, function(work){
		...
		db.query(
			"UPDATE accounts SET status=?, lat=?, lon=?, city= ?, message = ?, last_time=now() WHERE username=?",
			[work.status, work.lat, work.lon, work.city, work.message, work.username],
			function (err){
				if (err) throw err;	
				//res.writeHead(200, {"Content-Type": "application/json"});
				//var json = JSON.stringify({
	...
	exports.parseReceivedData(req, function (work){
		...
		function(err, rows){
			...
			for (var i = 0; i < rows.length; i++){
				var dt = exports.calculateGPSDistance(work.lat, work.lon, rows[i].lat, rows[i].lon);
				if (  dt < 100)
				{
					res.writeHead(200, {"Content-Type": "application/json"});
					var json = JSON.stringify({
						...
						time 	: rows[i].time,
						last_time : rows[i].last_time,
						description : rows[i].description,
						distance : dt
						});
					console.log('Notify: ', json);
	  				res.end(json);	
					break;
				}
			}
...
exports.help_others = function (db, req, res){
	exports.parseReceivedData(req, function (work){
		...
		console.log(work);
		db.query(
			"SELECT username, lat, lon, city, message, name, phone FROM accounts WHERE status = 1 AND username <> ?",
			[work.username],
  		...
  		// 		res.end(json);	
				...
				console.log("Condition ", work.distance);
				for (var i = 0; i < rows.length; i++)
				{
				...
				if (weak.length > 0 ){
					...
					for (var i = 0; i < weak.length - 1; i++)
						json += JSON.stringify(weak[i]) + ',';

					json += JSON.stringify(weak[weak.length - 1])+ ']}';

					console.log("Received ", weak.length, " user");
